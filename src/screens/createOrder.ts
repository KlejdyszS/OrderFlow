import { getTeam, getInventory, createOrder } from '../data/store';
...
export async function renderCreateOrder(): Promise<HTMLElement> {
  ...
  `;
    ...
    async function submit() {
        ...
        try {
            const ord = await createOrder({ clientName: cn, clientAddress: addr, description: desc, priority: pri, deadline: new Date(dead).toISOString(), assignedTo: assign, variants: rows });
